...
split KV的话会导致O需要存两份，因为后续需要进行累加
不复用 sO 的话:
...
kv_stage = 1 意味着零重叠:MMA 每算一块都要干等 TMA——"流水级数就没了"说的就是这个。

开 overlap_sO_sQ 后,sO 骑在 sQ 上(:353 取 max(96, 64) = 96KB 而非 sum):
